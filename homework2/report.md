...
本題實作一個外部排序分析工具：模擬在磁碟 I/O 延遲存在的情況下進行多路合併排序（k-way merge），並計算總輸入時間 t_input 隨合併路數 k 改變的影響。可應用於大數據處理、磁碟排序優化、資料倉儲查詢加速等場景。

---

本模擬以一個多階段排序流程為基礎，其中初始階段產生多個排序好的暫存 run，接著進行 k-way 合併排序。
假設硬碟每次讀取需耗費固定的 seek 時間與 latency，並對每筆記錄產生傳輸成本，因此 t_input 時間可由下式估算：t_input = n × t_t + log_k(m) × m × (t_s + t_l)
n：總記錄數
...
程式會針對不同的 k 值（2、4、8、...、64）計算 t_input，並與固定 CPU 處理時間 t_CPU = 5 秒 作比較，觀察資料傳輸是否成為瓶頸。
